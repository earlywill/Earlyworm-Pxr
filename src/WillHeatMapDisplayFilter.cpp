...
// Turbo LUT https://ai.googleblog.com/2019/08/turbo-improved-rainbow-colormap-for.html
...
RtColorRGB interpolate ( float value )
{
    RtColorRGB result;
    float x = std::max (0.f, std::min(1.f, value));
    int a = int(x * 255);
    int b = int(std::min(255.f, a + 1.f));
    float f = x*255.f - a;
    for (int i=0; i<3; i++)
        result[i] = turbo_srgb_floats[a][i] + (turbo_srgb_floats[b][i] - turbo_srgb_floats[a][i]) * f;
    return result;
}
...
class WillHeatMapDisplayFilter : public RixDisplayFilter
{

public:
    WillHeatMapDisplayFilter();
    virtual ~WillHeatMapDisplayFilter();
...
void
WillHeatMapDisplayFilter::dspyParams::Free(RtPointer dataPtr)
{
    dspyParams *data = static_cast<dspyParams*>(dataPtr);
    delete data;
}

WillHeatMapDisplayFilter::WillHeatMapDisplayFilter()
{
}
...
int WillHeatMapDisplayFilter::Init(RixContext &ctx, RtUString const pluginPath)
{
    PIXAR_ARGUSED(ctx);
...
WillHeatMapDisplayFilter::GetParamTable()
{
    static RixSCParamInfo s_ptable[] =
    {
        ...
        RixSCParamInfo(RtUString("D"), k_RixSCFloat),
        RixSCParamInfo(RtUString("turboLut"), k_RixSCInteger),
        RixSCParamInfo(RtUString("readAov"), k_RixSCString),
        RixSCParamInfo(RtUString("writeAov"), k_RixSCString),
        RixSCParamInfo()
    };
    return &s_ptable[0];
}
    
void WillHeatMapDisplayFilter::Finalize(RixContext &ctx) 
{
    PIXAR_ARGUSED(ctx);
}

void WillHeatMapDisplayFilter::CreateInstanceData(RixContext& ctx, RtUString const handle,
...
void WillHeatMapDisplayFilter::initializeInstanceData(RixContext& ctx,
                                                              ...
                                                              InstanceData* instance)
{
    ...
    p->D = 0.8f; // luminance
    p->turboLut = 0;
    RtUString readAov("sampleCount");
    RtUString writeAov("heatMap");

    parms->EvalParam( k_A, 0, &p->A);
    parms->EvalParam( k_B, 0, &p->B);
    parms->EvalParam( k_C, 0, &p->C);
    parms->EvalParam( k_D, 0, &p->D);
    parms->EvalParam( k_turboLut, 0, &p->turboLut);

    parms->EvalParam( k_readAov, 0, &readAov );
    parms->EvalParam( k_writeAov, 0, &writeAov );
    ...
    // retrieving read channel first
    RixDisplayChannel const *readChannel =
        integratorEnv->GetDisplayChannel(readAov);

    if (readChannel)
    {
        p->readId = readChannel->id;
        p->readFloat =
            (readChannel->type == RixDisplayChannel::k_FloatChannel ||
             readChannel->type == RixDisplayChannel::k_IntegerChannel);

        p->writeChannels.clear();
        ...
        if (p->writeChannels.empty())
        {
            msg->Warning( "PxrCopyAOVDisplayFilter '%s' did not find any display "
                          "to write to for Aov '%s'",
                          handle.CStr(), writeAov.CStr() );
            return;
        }

        p->writeFloat =
            (p->writeChannels[0].type == RixDisplayChannel::k_FloatChannel ||
             p->writeChannels[0].type == RixDisplayChannel::k_IntegerChannel);
    }
    else
    {
        msg->Warning( "PxrCopyAOVDisplayFilter '%s' did not find a display "
                      "to read from for Aov '%s'",
                      handle.CStr(), readAov.CStr() );
        return;
...
void WillHeatMapDisplayFilter::SynchronizeInstanceData(
...
void WillHeatMapDisplayFilter::Filter(RixDisplayFilterContext &fCtx,
                                              RtPointer instanceData)
{
    ...
    if (!p->readFloat && !p->writeFloat) // color to color
    {
        for (int x = fCtx.xmin; x < fCtx.xmax; ++x) {
            for (int y= fCtx.ymin; y < fCtx.ymax; ++y) {
                RtColorRGB aovPixel;
                fCtx.Read(p->readId, x, y, aovPixel);
                std::cout << "c2c" << std::endl;
                for (int d = 0; d < p->writeChannels.size(); ++d)
                {
                    fCtx.Write(p->writeChannels[d].id, x, y, aovPixel);
                }
            }
        }
    }
    else if (!p->readFloat) // color to float
    {
        for (int x = fCtx.xmin; x < fCtx.xmax; ++x) {
            for (int y= fCtx.ymin; y < fCtx.ymax; ++y) {
                RtColorRGB aovPixel;
                ...
                for (int d = 0; d < p->writeChannels.size(); d++)
                {
                    fCtx.Write(p->writeChannels[d].id, x, y, aovPixel.ChannelAvg());
                }
            }
        }
    }
    else if (!p->writeFloat) // float to color
    {
        for (int x = fCtx.xmin; x < fCtx.xmax; ++x) {
            for (int y= fCtx.ymin; y < fCtx.ymax; ++y) {
                float aovPixel;
                ...
                for (int d = 0; d < p->writeChannels.size(); d++)
                {
                    aovPixel = RixInputRange (aovPixel, p->A, p->B);
                    // use lut or heatmap
                    if (p->turboLut > 0)
                    {
                        pixel = interpolate(aovPixel);
                    }
                    else
                    {
                        aovPixel *= 0.66f;
                        RixHsvToRgb(RtColorRGB(aovPixel, p->C, p->D), pixel); 
                    }
                    fCtx.Write(p->writeChannels[d].id, x, y, pixel);
                }
            }
        }
    }
    ...
    for (int d = 0; d < p->channels.size(); ++d)
    {
        for (int x = fCtx.xmin; x < fCtx.xmax; ++x) {
            for (int y= fCtx.ymin; y < fCtx.ymax; ++y) {
                ...
                pixel = RixInputRange (pixel, 4.f, 256.f);
                pixel[0] *= 0.66f;
                pixel[1] = 0.8f;
                pixel[2] = 0.8f;
                RixHsvToRgb(pixel, pixel);    
...
{
    PIXAR_ARGUSED(hint);
    return new WillHeatMapDisplayFilter();
}

RIX_DISPLAYFILTERDESTROY
{
    delete reinterpret_cast< WillHeatMapDisplayFilter * >( filter );
}
